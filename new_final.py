...
def main():
    ...
    if st.session_state.feature_selection == "风格迁移" and hasattr(st.session_state,
                                                                    "style_transfer_mode") and st.session_state.style_transfer_mode == "手动提供范文":

        if "reference_text" not in st.session_state:
            if reference_text := st.chat_input("范文：(请输入两次，第一次输入范文，第二次输入任意字符）"):
                st.session_state.reference_text = reference_text
                st.session_state[history_key].append({"type": "风格迁移", "input": f"范文：{reference_text}"})
        else:
            if input_text := st.chat_input("待修改文章："):
                st.session_state[history_key].append({"type": "风格迁移", "input": f"待修改文章：{input_text}"})
